...
/**
 * Store an already-signed admin session token (generated by backend)
 */
export function storeAdminSession(
  token: string,
  info: Omit<AdminSessionInfo, 'createdAt'> & { createdAt?: number }
): void {
  const createdAt = info.createdAt ?? Date.now();
  sessionStorage.setItem(ADMIN_TOKEN_KEY, token);
  sessionStorage.setItem(
    ADMIN_INFO_KEY,
    JSON.stringify({ email: info.email, role: info.role, createdAt })
  );
}
...
export async function createAdminSession(
  ...
  storeAdminSession(token, { email, role, createdAt: timestamp });
