...
const getCacheKey = (slug?: string) => `whitelabel_cache_v2_${normalizeSlug(slug) || getSlugFromPath()}`;
  ...
  useEffect(() => {
    ...
    const loadFromDb = async () => {
      try {
        ...
        // Load the youtube_channel config saved during registration (store-specific ONLY)
        if (storeId) {
