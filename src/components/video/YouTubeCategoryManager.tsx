...
export function YouTubeCategoryManager({
  ...
  const addCategory = () => {
    ...
    onChange({
      ...
      categories: [
        ...draft.categories,
        { id, name, icon: newIcon.trim() || "🎬", order: draft.categories.length + 1, keywords: [] },
      ],
    });
    setNewName("");
    setNewIcon("🎬");
  ...
  const renameCategory = (categoryId: string) => {
    ...
    if (!trimmed) { setEditingCatId(null); return; }
    onChange({
      ...draft,
  ...
  const addKeyword = (categoryId: string) => {
    ...
    onChange({
      ...draft,
      categories: draft.categories.map((c) =>
        c.id === categoryId ? { ...c, keywords: [...(c.keywords || []), kw] } : c
      ),
    });
  ...
  const removeKeyword = (categoryId: string, keyword: string) => {
    onChange({
      ...draft,
      categories: draft.categories.map((c) =>
        c.id === categoryId ? { ...c, keywords: (c.keywords || []).filter((k) => k !== keyword) } : c
      ),
    });
  };
  ...
  const setMapping = (videoId: string, categoryId?: string) => {
    onChange({
      ...draft,
      videoCategoryMap: { ...draft.videoCategoryMap, [videoId]: categoryId },
    });
  };
    ...
    <GlassCard className="p-6">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-display font-semibold text-lg">{t("admin.videoCategories")}</h3>
            <p className="text-sm text-muted-foreground">
              {t("admin.categoriesOrganized", { count: categoriesSorted.length, videos: categorizedCount })}
            </p>
          </div>
        </div>
            ...
            <div className="pt-6 border-t border-border mt-6">
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  />
                  <Label className="text-sm">{t("admin.autoCategorize")}</Label>
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  <Label>{t("admin.createCategory")}</Label>
                  <div className="flex flex-col sm:flex-row gap-2">
                    <Input value={newIcon} onChange={(e) => setNewIcon(e.target.value)} className="sm:w-24" placeholder="🎭" aria-label="Icon" />
                ...
                <AnimatePresence>
                  {autoCategorizeEnabled && categoriesSorted.length > 0 && (
                    ...
                    >
                      <Label>{t("admin.categoriesKeywords")}</Label>
                      <div className="space-y-3">
                        {categoriesSorted.map((cat) => (
                          <div key={cat.id} className="p-4 rounded-lg bg-muted/30 border border-border/50 space-y-3">
                            <div className="flex items-center justify-between">
                              {editingCatId === cat.id ? (
                                <div className="flex items-center gap-2 flex-1 mr-2">
                                  ...
                                  <Input
                                    ...
                                    onKeyDown={(e) => { if (e.key === "Enter") renameCategory(cat.id); }}
                                    className="h-8 text-sm"
                                    placeholder={t("admin.renamePlaceholder")}
                                    autoFocus
                                  />
                                  <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => renameCategory(cat.id)}>
                                    <Check className="w-4 h-4 text-primary" />
                                  </Button>
                                  <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => setEditingCatId(null)}>
                              ...
                              <Button variant="ghost" size="sm" onClick={() => removeCategory(cat.id)} className="h-7 w-7 p-0 text-muted-foreground hover:text-destructive">
                                <X className="w-4 h-4" />
                              </Button>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {(cat.keywords || []).map((kw) => (
                                <Badge key={kw} variant="secondary" className="gap-1 pr-1 cursor-pointer hover:bg-destructive/20" onClick={() => removeKeyword(cat.id, kw)}>
                                  {kw}
                                  <X className="w-3 h-3" />
                                </Badge>
                              ))}
                            ...
                            <div className="flex gap-2">
                              <Input
                                ...
                                onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addKeyword(cat.id); } }}
                                placeholder={t("admin.addKeyword")}
                ...
                {/* Video list */}
                <div className="space-y-3">
                  <Label>{t("admin.linkVideos")}</Label>
                  <div className="relative">
                    <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" aria-hidden />
                    <Input value={query} onChange={(e) => setQuery(e.target.value)} className="pl-9" placeholder={t("admin.searchVideos")} aria-label={t("admin.searchVideos")} />
                  </div>
                  ...
                  <div className="space-y-2">
                    {visibleVideos.map((v, idx) => {
                      ...
                      return (
                        ...
                        >
                          <img src={v.thumbnail_url} alt={v.video_title} className="w-20 aspect-video rounded-md object-cover" loading="lazy" />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium line-clamp-1">{v.video_title}</p>
                            <p className="text-xs text-muted-foreground">{new Date(v.published_at).toLocaleDateString("en-US")}</p>
                          </div>
                          <div className="w-44">
                            <Select value={selected} onValueChange={(val) => setMapping(v.video_id, val === NONE_VALUE ? undefined : val)}>
                    ...
                    {!visibleVideos.length && (
                      <p className="text-xs text-muted-foreground">{t("admin.noVideosFound")}</p>
                    )}

                    {!query.trim() && videos.length > 40 && (
                      <p className="text-xs text-muted-foreground">{t("admin.showing", { count: 40 })}</p>
                    )}

                    {query.trim() && filteredVideos.length > 80 && (
                      <p className="text-xs text-muted-foreground">{t("admin.showingResults", { count: 80 })}</p>
                    )}
                  </div>
                </div>
              </div>
