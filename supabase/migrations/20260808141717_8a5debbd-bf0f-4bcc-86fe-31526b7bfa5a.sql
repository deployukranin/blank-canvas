...
BEGIN
  ...
  v_plan_norm := lower(trim(v_plan));
  v_trial := (v_plan_norm IN ('trial', 'free', 'none'));
  ...
  v_limit := CASE
    ...
    WHEN v_plan_norm IN ('premium', 'enterprise') THEN 0
    WHEN v_plan_norm IN ('pro', 'profissional', 'professional') THEN 50::bigint * 1024 * 1024 * 1024
    -- any other paid plan (including 'paid', 'basic', 'basico') gets 15 GB
    ELSE 15::bigint * 1024 * 1024 * 1024
  END;
